//! Footnote extension for mordant.
//!
//! Parses PHP Markdown Extra style footnotes and renders them as HTML
//! with superscript references and end-of-document footnote list.
//!
//! # Syntax
//! ```markdown
...
use rushdown_lib::parser::{self, AnyBlockParser, AnyInlineParser, BlockParser, InlineParser, NoParserOptions, Parser, ParserExtension, ParserExtensionFn, PRIORITY_LINK, PRIORITY_LIST};
...
use rushdown_lib::renderer::html::{Renderer, RendererExtension, RendererExtensionFn};
use rushdown_lib::text::{self, Reader};
use rushdown_lib::util::{indent_position, is_blank};
...
struct FootnoteDefinitionParser {
    footnote_list: rushdown_lib::context::ContextKey<rushdown_lib::context::ObjectValue>,
}

impl FootnoteDefinitionParser {
    pub fn new(reg: Rc<RefCell<rushdown_lib::context::ContextKeyRegistry>>) -> Self {
        let footnote_list = reg.borrow_mut().get_or_create::<rushdown_lib::context::ObjectValue>(FOOTNOTE_LIST);
        Self { footnote_list }
    }
}
...
struct FootnoteReferenceParser {
    footnote_list: rushdown_lib::context::ContextKey<rushdown_lib::context::ObjectValue>,
    reference_list: rushdown_lib::context::ContextKey<rushdown_lib::context::ObjectValue>,
}
...
impl FootnoteReferenceParser {
    pub fn new(reg: Rc<RefCell<rushdown_lib::context::ContextKeyRegistry>>) -> Self {
        ...
        let reference_list = reg
            .borrow_mut()
            .get_or_create::<rushdown_lib::context::ObjectValue>(REFERENCE_LIST);
        Self {
            footnote_list,
            reference_list,
        }
...
impl<W: TextWrite> FootnoteReferenceHtmlRenderer<W> {
    fn new(
        _reg: Rc<RefCell<rushdown_lib::context::ContextKeyRegistry>>,
...
struct FootnoteDefinitionHtmlRenderer<W: TextWrite> {
    _phantom: PhantomData<W>,
    footnote_list: rushdown_lib::context::ContextKey<rushdown_lib::context::ObjectValue>,
    footnote_render: rushdown_lib::context::ContextKey<rushdown_lib::context::BoolValue>,
}

impl<W: TextWrite> FootnoteDefinitionHtmlRenderer<W> {
    pub fn new(reg: Rc<RefCell<rushdown_lib::context::ContextKeyRegistry>>) -> Self {
        let footnote_list = reg.borrow_mut().get_or_create::<rushdown_lib::context::ObjectValue>(FOOTNOTE_LIST);
        let footnote_render = reg.borrow_mut().get_or_create::<rushdown_lib::context::BoolValue>(FOOTNOTE_RENDER);
        Self {
            _phantom: PhantomData,
            footnote_list,
            footnote_render,
...
impl<W: TextWrite> FootnotePostRenderHook<W> {
    pub fn new(
        reg: Rc<RefCell<rushdown_lib::context::ContextKeyRegistry>>,
        html_opts: html::Options,
        options: FootnoteHtmlRendererOptions,
    ) -> Self {
        let footnote_list = reg.borrow_mut().get_or_create::<rushdown_lib::context::ObjectValue>(FOOTNOTE_LIST);
...
impl PyFootnoteHtmlRendererOptions {
    pub fn to_rushdown(&self) -> FootnoteHtmlRendererOptions {
