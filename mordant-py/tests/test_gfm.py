...
|---|---|
| 1 | 2 |"""
    html = mordant.markdown_to_html(md, gfm=False)
...
def test_gfm_strikethrough_disabled():
    html = mordant.markdown_to_html("~~deleted~~", gfm=False)
...
def test_gfm_linkify_url():
    html = mordant.markdown_to_html("Visit https://example.com today", gfm=True)
    assert '<a href="https://example.com">' in html


def test_gfm_linkify_disabled():
    html = mordant.markdown_to_html("Visit https://example.com today", gfm=False)
