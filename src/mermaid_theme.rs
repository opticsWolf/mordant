...
/// Optional lookup for syntect code-highlighting themes by name.
///
/// The core crate does not depend on any specific theme registry; hosts that
/// have one (e.g. the `highlighter` module in mordant-py, or a future core
/// `highlighter` feature) register a resolver via
/// [`register_syntax_theme_lookup`]. Unregistered → named code themes are not
/// resolvable and `resolve_mermaid_theme` falls back to [`MermaidThemeSpec::None`].
static SYNTAX_THEME_LOOKUP: std::sync::RwLock<Option<fn(&str) -> Option<Theme>>> =
    std::sync::RwLock::new(None);

/// Register the resolver used to turn a theme name into a syntect
/// code-highlighting theme (step 2 of [`resolve_mermaid_theme`]).
pub fn register_syntax_theme_lookup(f: fn(&str) -> Option<Theme>) {
    *SYNTAX_THEME_LOOKUP.write().unwrap() = Some(f);
}

/// Resolve a theme name to a [`MermaidThemeSpec`].
///
/// 1. Built-in mermaid presets via `Theme::from_name` (native, no derivation).
/// 2. Otherwise a syntect code-highlighting theme via the registered lookup
///    (derived).
...
pub fn resolve_mermaid_theme(name: &str) -> MermaidThemeSpec {
    ...
    if let Some(lookup) = *SYNTAX_THEME_LOOKUP.read().unwrap() {
...
mod tests {
    ...
    fn derived_scheme_structure_and_contrast() {
        // Iterate the default syntect themes — no filesystem or host registry
        // needed; core has no built-in theme registry.
        let themes = syntect::highlighting::ThemeSet::load_defaults();
        assert!(!themes.themes.is_empty());
        for (_name, t) in &themes.themes {
    ...
    fn unknown_name_is_none() {
        // No syntax-theme lookup registered → named code themes can't resolve.
        assert!(matches!(
    ...
    #[test]
    fn registered_lookup_derives_theme() {
        register_syntax_theme_lookup(|name| {
            if name == "TestTheme" {
                Some(syntect::highlighting::ThemeSet::load_defaults()
                    .themes
        ...
        assert!(matches!(
            resolve_mermaid_theme("TestTheme"),
            MermaidThemeSpec::Derived(_)
        ));
    }
}
