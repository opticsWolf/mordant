//! LaTeX math rendering for mordant (core), backed by the pure-Rust `katex-rs`
//! crate. Pure engine; PyO3 wrappers live in mordant-py/src/math.rs.
//!
...
use crate::renderer::{self, html, NodeRenderer, RenderNode, TextWrite, NodeRendererRegistry, BoxRenderNode};
use crate::renderer::html::{RendererExtension, RendererExtensionFn};
...
pub fn output_from_str(s: &str) -> Option<OutputFormat> {
...
/// Render a LaTeX expression to markup.
///
/// `output` must be `"html"`, `"mathml"`, or `"both"`; results are cached.
pub fn render_math(latex: &str, display: bool, output: &str)
    -> std::result::Result<Arc<str>, String> {
    let fmt = output_from_str(output)
        .ok_or_else(|| format!("output must be 'html', 'mathml', or 'both', got {output:?}"))?;
    Ok(render_math_cached(latex, display, fmt))
}
...
impl crate::renderer::RendererOptions for MathRendererOptions {}
...
use crate::ast::{KindData, NodeKind, NodeType, PrettyPrint, pp_indent};
use core::fmt;
use core::fmt::Write as FmtWrite;
use crate::parser::{self, AnyInlineParser, InlineParser, Parser, ParserExtension, ParserExtensionFn, ParserOptions, PRIORITY_EMPHASIS};
use crate::text::Reader;
use crate::as_extension_data;
...
impl InlineParser for MathParser {
    ...
    fn parse(
        ...
        arena: &mut Arena,
        _parent_ref: NodeRef,
        reader: &mut crate::text::BlockReader,
...
impl crate::renderer::RendererOptions for MathInlineRendererOptions {}
